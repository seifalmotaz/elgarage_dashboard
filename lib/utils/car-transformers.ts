 ...
 */
export function mapPublicationStatus(status: Car['status']): PublicationStatusLabel {
  const statusMap: Record<Car['status'], PublicationStatusLabel> = {
    DRAFT: 'مسودة',
    PUBLISHED: 'منشورة',
    SOLD: 'مباعة',
  };
...
export interface CarCardProps {
  ...
  mileage: string;
  location: string;
  /** Publication status shown as the image ribbon badge */
...
export function transformCarToCard(car: Car): CarCardProps {
  ...
  return {
    ...
    location: car.address || 'غير محدد',
    status: mapPublicationStatus(car.status),
