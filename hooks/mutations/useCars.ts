...
import { carsApi, CarStatus, type CarsListResponse } from '@/lib/api/cars';
import { queryKeys } from '@/lib/query-keys';
import type { CreateCarDto, UpdateCarDto } from '@/lib/api/types';
import { toast } from 'react-hot-toast';
import { logger } from '@/lib/logger';

const carListQueryKey = ['cars', 'list'] as const;
const infiniteCarListQueryKey = ['cars', 'infinite'] as const;

function updateFeaturedCar(
  data: CarsListResponse | undefined,
  id: string,
  isFeatured: boolean,
) {
  if (!data) return data;
  ...
  return {
    ...
    data: data.data.map((car) =>
      car.id === id
        ? { ...car, isFeatured, featuredAt: isFeatured ? new Date().toISOString() : null }
        : car,
    ),
  };
}
...
export function useToggleFeaturedMutation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, isFeatured }) => {
      await Promise.all([
        queryClient.cancelQueries({ queryKey: queryKeys.cars.detail(id) }),
        queryClient.cancelQueries({ queryKey: carListQueryKey }),
        queryClient.cancelQueries({ queryKey: infiniteCarListQueryKey }),
      ]);
      const previousCar = queryClient.getQueryData(queryKeys.cars.detail(id));
      const previousLists = queryClient.getQueriesData<CarsListResponse>({ queryKey: carListQueryKey });
      const previousInfiniteLists = queryClient.getQueriesData<InfiniteData<CarsListResponse>>({
        queryKey: infiniteCarListQueryKey,
      });
      ...
      queryClient.setQueriesData<CarsListResponse>({ queryKey: carListQueryKey }, (old) =>
        updateFeaturedCar(old, id, isFeatured),
      );
      queryClient.setQueriesData<InfiniteData<CarsListResponse>>(
        { queryKey: infiniteCarListQueryKey },
        (old) => old
          ? { ...old, pages: old.pages.map((page) => updateFeaturedCar(page, id, isFeatured)!) }
          : old,
      );

      return { previousCar, previousLists, previousInfiniteLists };
    },
    onError: (error: Error, { id }, context) => {
      if (context?.previousCar) {
        queryClient.setQueryData(queryKeys.cars.detail(id), context.previousCar);
      ...
      context?.previousInfiniteLists.forEach(([queryKey, data]) => queryClient.setQueryData(queryKey, data));
