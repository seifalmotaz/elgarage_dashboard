...
import FilterBar, {
  type CarsFilterTab,
  type CarsStatusFilter,
} from "../../../components/dashboard/cars/FilterBar";
import CarsTable from "../../../components/dashboard/cars/CarsTable";
import Button from "../../../components/ui/Button";
import Modal from "../../../components/ui/Modal";
import toast from "react-hot-toast";
import { useCars, useInfiniteCars } from "../../../hooks/queries/useCars";
...
function tabToStatusFilter(tab: CarsFilterTab): CarsStatusFilter {
...
function statusFilterToTab(status: CarsStatusFilter): CarsFilterTab {
...
export default function CarsPage() {
  ...
  // Filter states
  const [activeTab, setActiveTab] = useState<CarsFilterTab>("all");
  const [statusFilter, setStatusFilter] = useState<CarsStatusFilter>("all");
  ...
  // Build the shared filters used by table pagination and the grid's infinite loading.
  const baseFilters = useMemo<Omit<CarFilters, "page" | "limit">>(() => {
    const f: CarFilters = {};
    ...
    return f;
  }, [activeTab, statusFilter, debouncedSearch, debouncedMinPrice, debouncedMaxPrice, debouncedMinYear, debouncedMaxYear, debouncedMinMileage, debouncedMaxMileage]);

  const paginatedFilters = useMemo<CarFilters>(
    () => ({ ...baseFilters, page: currentPage, limit: ITEMS_PER_PAGE }),
    [baseFilters, currentPage],
  );

  const tableQuery = useCars(paginatedFilters, viewMode === "table");
  const gridQuery = useInfiniteCars(baseFilters, viewMode === "grid", ITEMS_PER_PAGE);
  const {
    fetchNextPage,
    hasNextPage: gridHasNextPage,
    isFetchingNextPage,
    isPlaceholderData: isGridPlaceholderData,
  } = gridQuery;
  const deleteCarMutation = useDeleteCarMutation();
  const toggleFeaturedMutation = useToggleFeaturedMutation();

  const latestGridPage = gridQuery.data?.pages[gridQuery.data.pages.length - 1];
  const cars = viewMode === "grid"
    ? gridQuery.data?.pages.flatMap((page) => page.data) || []
    : tableQuery.data?.data || [];
  const activePage = viewMode === "grid" ? latestGridPage : tableQuery.data;
  ...
  const isFetching = viewMode === "grid" ? gridQuery.isFetching : tableQuery.isFetching;
  const error = viewMode === "grid" ? gridQuery.error : tableQuery.error;
  ...
  // Handle tab change — keep status dropdown in sync for status tabs
  const handleTabChange = (tab: CarsFilterTab) => {
    setActiveTab(tab);
    if (tab === "special") {
      setStatusFilter("all");
    } else {
  ...
  // Handle status dropdown — keep tabs in sync
  const handleStatusFilterChange = (status: CarsStatusFilter) => {
    setStatusFilter(status);
    setActiveTab(statusFilterToTab(status));
    setCurrentPage(1);
  };
    ...
    <div className="flex flex-col gap-8 pb-10 w-full max-w-[1200px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <SummaryCard
          ...
          value={(meta?.total ?? stats.total).toString()}
      ...
      <div className="bg-white px-8 py-4 rounded-xl">
        ...
        {isFetching && !isFetchingNextPage && (
          <p className="text-sm text-gray-500" role="status">جارِ تحديث السيارات...</p>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {transformedCars.map((car) => (
                <CarCard
                  ...
                  onEdit={() => router.push(`/dashboard/cars/${car.id}/edit`)}
                />
              ))}
              {gridHasNextPage && !isGridPlaceholderData && (
                <div ref={loadMoreRef} className="col-span-full flex justify-center py-4">
                  ...
                  >
                    {isFetchingNextPage ? "جارِ تحميل المزيد..." : "تحميل المزيد"}
                  </Button>
                </div>
              )}
              {!gridHasNextPage && cars.length > 0 && (
                <p className="col-span-full py-4 text-center text-sm text-gray-500">تم عرض كل السيارات</p>
              )}
            </div>
          ) : (
            <CarsTable
              ...
              onEdit={(id) => router.push(`/dashboard/cars/${id}/edit`)}
              ...
              pagination={{
                ...
                totalItems: meta?.total || tableRows.length,
