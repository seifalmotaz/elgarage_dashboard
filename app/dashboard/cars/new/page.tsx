...
import { useCarSpecs, useCarFeatures, useCarBrands } from "../../../../hooks/queries/useCars";
import { InspectionSelector } from "@/components/dashboard/cars/InspectionSelector";
import { ImageReorderGrid } from "@/components/dashboard/cars/ImageReorderGrid";
import CarImageUploader from "@/components/dashboard/cars/CarImageUploader";
import type { InspectionDetails } from "@/lib/api/cars";
...
export default function NewCarPage() {
  // Form hook
  const { form, formData, isSubmitting, submit, addImage, removeImage, updateBrandName, updateModelName, importFromInspection, clearImport, reorderImages } = useCarForm();

  // State for inspection import
  const [selectedInspection, setSelectedInspection] = useState<string>('');
  const [inspectionLoaded, setInspectionLoaded] = useState(false);

  // Dynamic data hooks (using React Query)
  ...
  const [importType, setImportType] = useState<"manual" | "auto">("manual");
  ...
  // Loading state
  const isLoading = specsLoading || featuresLoading || brandsLoading;
    ...
    <div className="flex flex-col gap-8 pb-20 w-full" dir="rtl">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2 text-[14px]">
           ...
           <span className="text-[#8286ab] font-light">معروضات جراج</span>
           <div className="opacity-40">
              <img src="/assets/dashboard/cars/car-arrow-left.svg" alt="arrow" width={10} height={10} />
           </div>
           <span className="text-[#111] font-semibold">اضافة سيارة جديدة</span>
        </div>
        <h1 className="text-[32px] font-bold text-[#111] text-start">اضافة سيارة جديدة</h1>
      </div>
      ...
      <div className="bg-white rounded-[16px] p-4 shadow-sm border border-[#f2f2f2] flex flex-col gap-5">
        <div className="border-b border-[#f2f2f2] pb-3">
          <p className="text-[16px] font-semibold text-[#002ec1] text-start">إضافة سيارة لمعروضات جراج</p>
        </div>
        <div className="flex flex-col md:flex-row gap-3">
          {/* Manual Option (Selected by default - for adding new cars manually) */}
          <div
            onClick={() => {
              setImportType("manual");
              // Clear any imported data when switching to manual
              if (inspectionLoaded) {
                clearImport();
            ...
            className={`flex-1 cursor-pointer rounded-[12px] p-8 flex flex-col items-center justify-center gap-3 border transition-all ${importType === "manual" ? 'border-[#002ec1] bg-white shadow-sm' : 'border-[#f2f2f2] bg-white hover:bg-gray-50'}`}
          >
            <img src="/assets/dashboard/cars/plus-large.svg" alt="Add" width={28} height={28} className={importType === "manual" ? "" : "opacity-40"} />
            <div className="flex flex-col items-center gap-1">
              <span className={`text-[16px] font-medium ${importType === "manual" ? 'text-[#002ec1]' : 'text-[#6b7280]'}`}>إضافة بيانات جديدة كلياً</span>
              <span className="text-[12px] text-[#8286ab] font-light text-center">إدخال جميع بيانات بشكل يدوي لرفعها كمعروض جديد.</span>
            </div>
          </div>

          {/* Auto Option - Import from inspection */}
          <div
            onClick={() => setImportType("auto")}
            className={`flex-1 cursor-pointer rounded-[12px] p-8 flex flex-col items-center justify-center gap-3 border transition-all ${importType === "auto" ? 'border-[#002ec1] bg-white shadow-sm' : 'border-[#f2f2f2] bg-white hover:bg-gray-50'}`}
          >
            <img src="/assets/dashboard/cars/shield-tick.svg" alt="Shield" width={28} height={28} className={importType === "auto" ? "" : "opacity-40"} />
            <div className="flex flex-col items-center gap-1">
              <span className={`text-[16px] font-semibold ${importType === "auto" ? 'text-[#002ec1]' : 'text-[#6b7280]'}`}>استيراد من الفحص</span>
              <span className={`text-[12px] font-light text-center ${importType === 'auto' ? 'text-[#4b5563]' : 'text-[#8286ab]'}`}>استيراد سيارة تم فحصها مسبقاً وتعبئة بياناتها</span>
            </div>
          </div>
        </div>
      </div>

      {/* 2. Media & General Info Section */}
      <div className="bg-white rounded-[16px] p-4 shadow-sm border border-[#f2f2f2] flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_500px] gap-8 items-start w-full min-w-0">
          {/* Form Column */}
          <div className="flex flex-col gap-5 min-w-0 w-full">
            {/* Show inspection selection only when "auto" is selected */}
            {importType === "auto" && (
              <div className="bg-white rounded-[16px] p-4">
                <label className="block mb-2 text-[14px] font-medium text-[#1a1a1a]">
                  اختر من السيارات المعتمدة
                </label>
                <InspectionSelector
                  value={selectedInspection}
                  onChange={(id) => setSelectedInspection(id)}
                  onInspectionLoad={(inspection: InspectionDetails) => {
                    // Import inspection data into form (year, mileage, location, images)
                    importFromInspection(inspection);
                    setInspectionLoaded(true);

                    // Auto-match brand and model from inspection brand/model names to dropdown IDs
                    const matchingBrand = brands.find(
                      (b) => b.name.toLowerCase() === inspection.brand.toLowerCase() ||
                             b.nameEn?.toLowerCase() === inspection.brand.toLowerCase()
                    );

                    if (matchingBrand) {
                      // Set brand ID - the useEffect will sync the brand name
                      form.setValue("brandId", matchingBrand.id);

                      // Find and set matching model
                      const matchingModel = matchingBrand.models.find(
                        (m) => m.name.toLowerCase() === inspection.model.toLowerCase() ||
                               m.nameEn?.toLowerCase() === inspection.model.toLowerCase()
                      );

                      if (matchingModel) {
                        // Set model ID - the useEffect will sync the model name
                        form.setValue("modelId", matchingModel.id);
                      }
                    }

                    toast.success('تم تحميل بيانات الفحص بنجاح');
                  }}
                  onError={(error) => {
            ...
            {/* Clear import button for auto mode */}
            {importType === "auto" && inspectionLoaded && (
              <button
                type="button"
                onClick={() => {
                  clearImport();
                  setSelectedInspection('');
                  setInspectionLoaded(false);
                  toast.success('تم مسح البيانات المستوردة');
                }}
                className="w-full py-3 text-[14px] text-red-600 hover:text-red-700 hover:underline transition-colors"
              >
                إلغاء الاستيراد والبدء من جديد
              </button>
            )}
            ...
            <Select
              ...
              onChange={(v) =>
                form.setValue("status", v as "DRAFT" | "PUBLISHED" | "SOLD", {
                  shouldDirty: true,
                })
              }
            />
          ...
          {/* Media Column */}
          <div className="flex flex-col gap-3 order-2 w-full lg:w-[500px] min-w-0 shrink-0">
            {/* Images Section */}
            <div className="bg-white rounded-[16px] p-4">
              ...
              {/* Clear imported images button (only in auto mode) */}
              {importType === "auto" && inspectionLoaded && uploadedImages.length > 0 && (
                <button
                  type="button"
                  onClick={() => {
                    clearImport();
...
{item.iconUrl ? (
      ...
      <div className="flex items-center gap-4 justify-start">
         <Button
           ...
           onClick={() => {
             console.log('[Page] Submit button clicked');
             submit();
           }}
           disabled={isSubmitting}
         >
           {isSubmitting ? 'جاري الإضافة...' : 'اضافة'}
         </Button>
