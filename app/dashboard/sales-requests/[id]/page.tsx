...
import { useCompleteInspectionMutation } from "@/hooks/mutations/useAdminInspections";
...
export default function RequestDetailsPage() {
  ...
  const completeInspectionMutation = useCompleteInspectionMutation();
  ...
  const onCompleteInspection = async () => {
    const reportId = data?.inspectionReport?.id;
    if (!reportId) return;
    if (!confirm('هل أنت متأكد من إكمال تقرير الفحص؟ سيصبح الطلب جاهزاً للاعتماد.')) return;
    await completeInspectionMutation.mutateAsync(reportId);
  };
  ...
  return (
    <div className="flex flex-col gap-6 pb-10" dir="rtl">
      ...
      <div className="flex flex-col gap-4">
        ...
        <InspectionReport
          report={data.inspectionReport}
          editUrl={`/dashboard/sales-requests/${id}/inspection/edit`}
        />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-3 border-t border-[#F2F2F2]">
            {data.status === "PENDING" && (
              <>
                <Button
                  ...
                  className="w-full sm:w-auto min-w-[190px] h-[44px] rounded-[200px] text-[12px] font-semibold"
                >
                  تعيين مفتش
                </Button>
                <Button
                  variant="outline"
                  onClick={() => router.push(`/dashboard/sales-requests/${id}/inspection/edit`)}
                  className="w-full sm:w-auto min-w-[190px] h-[44px] rounded-[200px] border-[#002ec1] text-[#002ec1] hover:bg-blue-50 text-[12px] font-semibold"
                >
                  {data.inspectionReport ? 'تعديل تقرير الفحص' : 'إدخال تقرير الفحص'}
                </Button>
                <Button
                  variant="outline"
                  onClick={onCancel}
            ...
            {data.status === "IN_INSPECTION" && (
              <>
                ...
                {data.inspectionReport?.id && (
                  <Button
                    variant="primary"
                    onClick={onCompleteInspection}
                    disabled={actionLoading}
                    className="w-full sm:w-auto min-w-[190px] h-[44px] rounded-[200px] bg-green-600 hover:bg-green-700 text-[12px] font-semibold"
                  >
                    {completeInspectionMutation.isPending
                      ? "جاري التنفيذ..."
                      : data.inspectionReport.status === "COMPLETED"
                        ? "رفع التقرير"
                        : "إكمال تقرير الفحص"}
                  </Button>
                )}
