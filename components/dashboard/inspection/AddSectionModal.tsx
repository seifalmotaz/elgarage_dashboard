...
interface AddSectionModalProps {
  ...
  onSubmit: (data: {
    ...
    titleEn?: string;
    icon?: string;
    options: { label: string; labelEn?: string; value: string; semanticType: OptionSemanticType }[];
...
const DEFAULT_OPTIONS: { label: string; labelEn?: string; semanticType: OptionSemanticType }[] = [
  { label: 'بدون خدوش', labelEn: 'No scratches', semanticType: 'GOOD' },
  { label: 'خفيفة', labelEn: 'Light', semanticType: 'WARN' },
  { label: 'واضحة', labelEn: 'Visible', semanticType: 'WARN' },
  { label: 'عميقة', labelEn: 'Deep', semanticType: 'BAD' },
];
...
export default function AddSectionModal({ isOpen, onClose, onSubmit, editSection }: AddSectionModalProps) {
  ...
  const [options, setOptions] = useState<{ label: string; labelEn?: string; semanticType: OptionSemanticType }[]>(DEFAULT_OPTIONS);
    ...
    if (editSection) {
      ...
      setOptions(
        editSection.options?.length > 0
          ? editSection.options.map((o) => ({ label: o.label, labelEn: o.labelEn ?? '', semanticType: o.semanticType }))
          : DEFAULT_OPTIONS
      );
  ...
  const handleAddOption = () => {
    setOptions([...options, { label: '', labelEn: '', semanticType: 'GOOD' }]);
  };
  ...
  const handleOptionLabelEnChange = (index: number, labelEn: string) => {
    const newOptions = [...options];
    newOptions[index] = { ...newOptions[index], labelEn };
    setOptions(newOptions);
  };
              ...
              <div key={index} className="flex items-center gap-2">
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    ...
                    className="bg-white border border-[#f2f2f2] h-[40px] rounded-[8px] px-3 text-[12px] text-[#1a1a1a] outline-none focus:border-[#002ec1]"
                  />
                  <input
                    type="text"
                    placeholder="Option label (English)"
                    value={option.labelEn || ''}
                    onChange={(e) => handleOptionLabelEnChange(index, e.target.value)}
                    dir="ltr"
                    className="bg-white border border-[#f2f2f2] h-[40px] rounded-[8px] px-3 text-[12px] text-[#1a1a1a] outline-none focus:border-[#002ec1] text-left"
                  />
                  <select
